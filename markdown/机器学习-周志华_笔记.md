...
均方误差对应了常用的欧几里得距离或简称欧式距离，基于均方误差最小化来进行模型求解的方法称为`最小二乘法`，![equation](http://latex.codecogs.com/gif.latex?(w^*,b^*)=argmin_{(w,b)}\sum_{i=1}^m(f(x_i)-y_i)^2) 最小化的过程，称为线性回归模型的最小二乘参数估计
